*
*     This is a version of SHGEQZ modified to use different deflation
*     criteria
*
      SUBROUTINE SHGEQZV1( JOB, COMPQ, COMPZ, N, ILO, IHI, H, LDH, T,
     $                   LDT,ALPHAR, ALPHAI, BETA, Q, LDQ, Z, LDZ, WORK,
     $                   LWORK, INFO, HDEFLATION, TDEFLATION, NIITER )
      IMPLICIT NONE
*
...
*
            GO TO 80
         ELSE
            IF( HDEFLATION .EQ. 1 ) THEN
               IF( ABS( H( ILAST, ILAST-1 ) ).LE.ATOL ) THEN
                  H( ILAST, ILAST-1 ) = ZERO
                  GO TO 80
               END IF
            END IF
            IF( HDEFLATION .EQ. 2 ) THEN
     ...
     $             + ABS( H( ILAST-1, ILAST-1 ) ) ) ) ) THEN
               ...
               END IF
            END IF
            IF( HDEFLATION .EQ. 3 ) THEN
               IF( ABS( H( ILAST, ILAST-1 ) ).LE.MAX( SAFMIN, ULP*( 
     $             ABS( H( ILAST, ILAST ) )
     $             + ABS( H( ILAST-1, ILAST-1 ) ) ) ) ) THEN
*                 Normwise criterion has passed, now check stricter criterion
                  TST1 = ABS(T(ILAST, ILAST)*H(ILAST-1,ILAST) 
     $                  - H(ILAST, ILAST)*T(ILAST-1,ILAST))
                  TST2 = ABS(T(ILAST, ILAST)*H(ILAST-1,ILAST-1) 
     $                  - H(ILAST, ILAST)*T(ILAST-1,ILAST-1))
                  AB = MAX( ABS(H(ILAST,ILAST-1)), TST1 )
                  BA = MIN( ABS(H(ILAST,ILAST-1)), TST1 )
                  AA = MAX( ABS(H(ILAST,ILAST)), TST2 )
                  BB = MIN( ABS(H(ILAST,ILAST)), TST2 )
                  S = AA + AB
                  IF( BA*( AB / S ).LE.MAX( SAFMIN,
     $                ULP*( BB*( AA / S ) ) ) ) THEN
                     H( ILAST, ILAST-1 ) = ZERO
                     GO TO 80
                  END IF
         ...
         END IF
*
         IF( TDEFLATION .EQ. 1 ) THEN
            IF( ABS( T( ILAST, ILAST ) ).LE.BTOL ) THEN
               T( ILAST, ILAST ) = ZERO
               GO TO 70
            END IF
         END IF
         IF( TDEFLATION .EQ. 2 ) THEN
     ...
     $             + ABS( T( ILAST-1, ILAST-1 )) ) ) ) THEN
            ...
            END IF
         END IF
         IF( TDEFLATION .EQ. 3 ) THEN
            IF( ABS( T( ILAST, ILAST ) ).LT.BTOL ) THEN
*              Normwise criterion has passed, now check stricter criterion
               IF( ABS( T(ILAST,ILAST) ) .LT. 
     $             MAX( SAFMIN, ULP*ABS(H(ILAST,ILAST)) ) ) THEN
                  ILAZR1 = .TRUE.
               END IF
            END IF
         END IF
...
*
     ...
     $            ABS( H( J, J ) ) + ABS( H( J-1, J-1 ) ) 
     $            ) ) ) THEN
*                    Normwise criterion has passed,
*                    now check stricter criterion
                     TST1 = ABS(T(J, J)*H(J-1,J) - H(J, J)*T(J-1,J))
                     TST1 = MIN(SAFMAX,TST1)
                     TST2 = ABS(T(J, J)*H(J-1,J-1) - H(J, J)*T(J-1,J-1))
                     TST2 = MIN(SAFMAX,TST2)
                     AB = MAX( ABS(H(J,J-1)), TST1 )
                     BA = MIN( ABS(H(J,J-1)), TST1 )
                     AA = MAX( ABS(H(J,J)), TST2 )
                     BB = MIN( ABS(H(J,J)), TST2 )
                     S = AA + AB
                     IF( BA*( AB / S ).LE.MAX( SAFMIN,
     $                  ULP*( BB*( AA / S ) ) ) ) THEN
                        H( J, J-1 ) = ZERO
...
*
     ...
     $             TEMP = TEMP + ABS ( T( J - 1, J ) )
               IF( ABS( T( J, J ) ).LT.MAX( SAFMIN,ULP*TEMP ) )
     $             ILAZR1 = .TRUE.
            END IF
            IF( TDEFLATION .EQ. 3 ) THEN
...
*                 Normwise criterion has passed, now check stricter criterion
                  TST1 = ABS(H(J,J))
                  IF( J.GT.ILO ) THEN
                     TST1 = TST1 + ABS( H( J,J-1 )*T( J,J-1 ) )
                  END IF
                  IF( J.LT.IHI ) THEN
                     TST1 = TST1 + ABS( H( J+1,J )*T( J+1,J ) )
                  END IF
                  IF( ABS( T(J,J) ) .LT. 
     $                MAX( SAFMIN, ULP*TST1 ) ) THEN
                     ILAZR1 = .TRUE.
                  END IF
               END IF
            END IF
